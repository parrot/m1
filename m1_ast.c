...
assign(char *lhs, char *rhs) {
    m1_assignment *a = (m1_assignment *)m1_malloc(sizeof(m1_assignment));
    return a;
}

m1_chunk *
chunk(int rettype, char *name, m1_expression *block) {
...
static m1_binexpr *
binexpr(m1_binop op) {
    m1_binexpr *e = (m1_binexpr *)m1_malloc(sizeof(m1_binexpr));
    e->op         = op;
    return e;    
}

void 
expr_set_binexpr(m1_expression *node, m1_expression *e1, 
                 m1_binop op, m1_expression *e2) 
{
    
    
    node->expr.b = binexpr(op);
    node->expr.b->left  = e1;
    node->expr.b->right = e2;    
...
expr_set_funcall(m1_expression *node, m1_funcall *f) {
    node->expr.f = f;
    node->type = EXPR_FUNCALL;   
}

m1_funcall *
funcall(char *name) {
    m1_funcall *f = (m1_funcall *)m1_malloc(sizeof(m1_funcall));
    f->name       = name;
    return f;   
}
