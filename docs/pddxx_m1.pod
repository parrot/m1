...
The register is pushed onto the regstack.
 
Then, the C<assignment> node is visited, which will allocate a new register
for C<x> if it hasn't been assigned one before. The assignment node can
pop off the register that was assigned to keep the value 42. 
